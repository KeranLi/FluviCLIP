...
#---------------------------Fixed modules--------------------------#
from models.Unet3D import RemoteSensingRegressionModel3D
from configs.Unet3D import Config
#---------------------------Changeable modules-----------------------------#
...
if __name__ == "__main__":
    ...
    # 数据划分
    train_size_high = int(0.7 * len(high_dataset))
    ...
    train_size_low = int(0.7 * len(low_dataset))
    ...
    # 初始化模型、优化器和早停机制
    high_model = RemoteSensingRegressionModel3D().to(config.device)
    high_optimizer = Adam(
    ...
    low_model = RemoteSensingRegressionModel3D().to(config.device)
    low_optimizer = Adam(
